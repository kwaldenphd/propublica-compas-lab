...
Once we have loaded the `reticulate` library (and likely installed `miniconda`), we can move between Python and RStudio syntax.
...
# compas_two_year_scores <- read.csv("data/compas-scores-two-years.csv")
...
```{python}
# load data from file
# compas_two_year_scores = pd.read_csv('data/compas-scores-two-years.csv', index_col=0)
...
```{python}
# cross tabs for recidivism rates by race
...
```{r}
# load data from CSV file
# compas_two_year_scores_violent <- read.csv("data/compas-scores-two-years-violent.csv")
...
```{python}
# load data from file
# compas_two_year_scores_violent = pd.read_csv('data/compas-scores-two-years-violent.csv', index_col=0)
...
import json, requests, urllib, urllib.request
urllib.request.urlretrieve("https://raw.githubusercontent.com/kwaldenphd/propublica-compas-lab/main/data/cox-violent-parsed.csv", "data/cox-violent-parsed.csv")
```
...
```{r}
# load data from CSV file
# cox_parsed <- read.csv("data/cox-parsed.csv")
...
```{python}
# load data from file
# cox_parsed = pd.read_csv('data/cox-parsed.csv', index_col=0)
